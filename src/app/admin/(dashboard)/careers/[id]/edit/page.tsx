...
export default function AdminCareersEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const item = await careersService.getById(id);
      if (!item) {
        addToast("Không tìm thấy vị trí", "error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await careersService.update(id, {
        title: form.title,
      ...
      addToast("Cập nhật vị trí thành công", "success");
      router.push("/admin/careers");
    } catch {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          >
            {submitting ? "Đang cập nhật..." : "Cập nhật vị trí"}
          </button>
        </div>
      </form>
    </div>
